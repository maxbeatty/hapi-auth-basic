...
internals.implementation = function (server, options) {
    ...
    var scheme = {
        authenticate: function (request, reply) {
            ...
            if (!authorization) {
                return reply(Boom.unauthorized(null, 'Basic', settings.basicAttributes));
            }
            ...
            if (!username && !settings.allowEmptyUsername) {
                return reply(Boom.unauthorized('HTTP authentication header missing username', 'Basic', settings.basicAttributes));
            }
            ...
            settings.validateFunc(request, username, password, function (err, isValid, credentials) {
                ...
                if (!isValid) {
                    return reply(Boom.unauthorized('Bad username or password', 'Basic', settings.basicAttributes), null, { credentials: credentials });
                }
